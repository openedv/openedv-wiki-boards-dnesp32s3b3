...
接下来是触摸初始化的核心程序，我们根据前面介绍的知识点，可以知道触摸的参数与屏幕大小和使用的触摸芯片有关，我们的 2.4 寸屏使用的是基合半导体的 CHSCxxx 系列触摸屏驱动 IC，这是一个 IIC 接口的驱动芯片，我们要编写 chscxxx系列芯片的初始化程序，并编写一个
坐标扫描程序，这里我们先预留这两个接口分别为 chscxxx_init()和 chscxxx_scan()，在 chscxxx.c文件中再专门实现这两个驱动，标记使用的为电容屏。
