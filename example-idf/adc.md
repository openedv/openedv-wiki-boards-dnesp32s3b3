...
1. 本章实验功能简介：使用 ADC2 采集通道2（GPIO13,该引脚已引出至开发板侧边的PORT2的TX引脚上）上面的电压，通过转换后换算成电压值。 LED闪烁，提示程序运行。
2. LED闪烁，指示程序正在运行。
 ...
 * @note        ESP32S3 ADC对噪声敏感,可能导致ADC读数出现较大偏差
...
从上面的代码中可以看出，调用adc_init()后，ADC将读取GPIO13引脚的模拟信号，并通过通道2转换为数字值。后续可通过adc_oneshot_read()获取该通道的原始数据。
...
void app_main(void)
{
    ...
    lcd_show_string(10, 110, 200, 16, 16, "ADC2_CH2_VAL:0", BLUE);
    lcd_show_string(10, 130, 200, 16, 16, "ADC2_CH2_VOL:0.000V", BLUE);
...
从上面的代码中可以看出，在进行完包括 ADC 的所有初始化工作后，便不断地获取 ADC2 通道 2 进行 20 次转换后经过均值滤波后的结果，并将该原始值显示在 LCD 上，同时还通过该电压的原始值计算出了电压的模拟量，并在 LCD 上进行显示。
