...
本章笔者讲解 lwIP 的 Socket接口如何配置 TCP服务器，并在此基础上实现收发功能。


本实验对应的工程文件夹为：`<开发板A盘路径>/4，程序源码/v_5.5版本例程/2，扩展例程-IDF版/2，WiFi例程/07_WiFi_TCPServer`。
...
在实现 TCP 协议之前，用户需要按照以下步骤配置结构体 sockaddr_in的成员变量，以便建立 TCPServer 连接：
...
<br />6,调用函数 bind 绑定本地 IP 地址和端口号。
<br />7,调用函数 listen 监听连接请求。
<br />8,调用函数 accept 监听连接。
<br />9,调用适当的收发函数来接收或发送数据。
...
程序启动后初始化并连接网络，配置TCP服务器。按键触发发送消息，接收数据后串口输出，send任务根据flag状态发送并清除标志，LEDR循环闪烁提示运行状态。
